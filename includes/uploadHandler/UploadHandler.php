...
class UploadHandler {
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'database' => $_SESSION['admin']['DBSCHEMA'],
            'host' => $_SESSION['admin']['DBSERVER'],
            'username' => $_SESSION['admin']['DBUSERNAME'],
            'password' => $_SESSION['admin']['DBPASSWORD'],
            // end

            'script_url' => $_SESSION['admin']['BASE_URL'] . 'includes/uploadHandler/',
            'upload_dir' => $_SESSION['admin']['BASE_PATH'] . 'userfiles/',
            'upload_url' => $_SESSION['admin']['BASE_URL'] . 'userfiles/',
...
//                    'max_width' => 350,
    ...
    protected function get_user_path() {
        if ($this->options['user_dirs']) {
            ...
            return 'H/'.$this->get_user_id().'/';
            else
            return 'M/'.$this->get_user_id().'/'; 
        }
        return '';
    }
...
//    }
    ...
    public function get($print_response = true) {
        ...
        unset($_SESSION['test']);
        $response = $this->getFilesFromId($response);
    ...
    public function post($print_response = true) {
        ...
        $files = array();
        if ($this->options['custom_name'] == TRUE) {
            //RENOMBRAR IMAGEN
            $file_name = 'file_' . str_replace('.', '', microtime(TRUE));
            //$file_name = $_SESSION['admin']['nombre'];
        }
    ...
    function add_img($whichimg) {
        $sql = 'INSERT INTO imagenes (imagen, id_pro, sexo) VALUES ("'.$whichimg.'", "'.$_SESSION['admin']['id'].'","'.$_SESSION['admin']['gender'].'")';
        $add_to_db = $this->query($sql) or die(mysql_error());
        return $add_to_db;
    }

    function delete_img($delimg) {
        $sql = 'DELETE FROM imagenes WHERE imagen = "' . $delimg . '" AND id_pro = "' . $_SESSION['admin']['id'] . '"';
        $delete_from_db = $this->query($sql) or die(mysql_error());
        return $delete_from_db;
    }
    
    function getFilesFromId($response){
//       $_SESSION['test']['$response'] = $response;
//        
//        $sql = 'SELECT imagen FROM imagenes WHERE id_pro = "'.$_SESSION['admin']['id'].'" AND sexo = "'.$_SESSION['admin']['gender'].'"';
...
//        }
//        
//        foreach ((array)$response['files'] as $key => $file){
//            if(!in_array($file->name, $imagenes)){
//                unset($response['files'][$key]);
//            }
//        }
//        
//        $response['files'] = array_values($response['files']);
//        $_SESSION['test']['return'] = $response;
